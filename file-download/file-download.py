...
arg_parser.add_argument("uuid", help="File uuid")
...
get_file_url = "{0}/file/v1/sites/{1}/files/{2}/url".format(server_url, args.site_id, args.uuid)
...
print ("get file {0} by url {1}".format(args.uuid, url))
response = session.get(url, headers=headers, stream=True)
response.raise_for_status()
with open('./{0}'.format(args.uuid), "wb") as handle:
